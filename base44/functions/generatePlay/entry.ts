  ...
  try {
    ...
    try {
      ...
      if (typeof rawText === 'string') {
        ...
        toParse = rawText.replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/```\s*$/i, '').trim();
      } else if (typeof rawText === 'object' && rawText !== null) {
        // InvokeLLM may return already-parsed object
        result = rawText;
      } else {
        toParse = String(rawText);
      }
      if (!result) result = JSON.parse(toParse);
    } catch (parseErr) {
      console.error('Parse error:', parseErr.message);
      console.error('Raw LLM output type:', typeof rawText);
    ...
    if (result?.diagram_data && !result?.diagram) {
      result.diagram = result.diagram_data;
    }
    if (result?.players && !result?.diagram) {
      result.diagram = { players: result.players, paths: result.paths || [], annotations: result.annotations || [] };
    }

    // Validate minimum structure
    if (!result?.play_meta || !result?.diagram) {
      console.error('Missing fields. Keys:', Object.keys(result || {}));
      console.error('Result snippet:', JSON.stringify(result).slice(0, 400));
      return Response.json({ error: 'AI returned incomplete structure. Please try again.' }, { status: 500 });
    }
